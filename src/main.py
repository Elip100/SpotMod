    ...
    while True:
        ...
        option_list(
            [
                ...
                "Manage backups",
                "SPACER",
                f"{Fore.RED}Uninstall SpotMod{Fore.GREEN}",
                "Quit",
                f"{Fore.BLUE}Update SpotMod Injector{Fore.GREEN}"
                if update_available
                else None,
            ],
            [
                ...
                uninstall,
                quit,
                partial(webbrowser.open, "https://github.com/Elip100/SpotMod/releases")
                if update_available
                else None,
            ],
        )
...
def manage_mods():
    while True:
        ...
        mod_option = option_list(
            [
                "Disable mod" if mod_enabled else "Enable mod",
                f"{Fore.RED}Remove mod{Fore.GREEN}",
                "SPACER",
                "Cancel",
            ]
        )
...
def manage_backups():
    while True:
        ...
        for backup in data:
            options.insert(
                0,
                datetime.strptime(backup["timestamp"], "%Y-%m-%d %H:%M:%S.%f").strftime(
                    "%A %B %d, %Y at %X"
                ),
            )
...
def create_backup():
    ...
    if inject.detect_spiceify(spotify_path):
        option_list(
            ["Back"],
            [manage_backups],
            "Backups cannot be created when Spotify is patched with Spicetify.\nUse Spicetify backups instead.",
        )
    option_list(
        [
            "Simple (only backs up files modified by SpotMod)",
            "Full (backs up entire Spotify installation)",
            "SPACER",
            "Cancel",
        ],
        [
            partial(inject.create_backup, "simple", True, spotify_path),
            partial(inject.create_backup, "full", True, spotify_path),
            None,
        ],
...
def manage_backup(backup):
    while True:
        ...
        if backup["mod"]:
            print(f"SpotMod version: {backup['ver']}")
        ...
        print(
            f"Spotify version: {backup['sver']} {f'{Fore.RED}(OLD | CURRENT VERSION: {spotify_ver}){Fore.GREEN}' if old else ''}\n"
        )
        option_list(
            ["Restore", "Delete", "SPACER", "Cancel"],
            [
                partial(restore_backup, backup, old),
                partial(delete_backup, backup),
                manage_backups,
            ],
        )
...
def not_detected():
    ...
    if len(json.load(open(f"{utils.datfolder}/data.json", "r"))["mods"]) > 0:
        utils.clear()
        option_list(
            ["Install saved mods", "Delete saved mods"],
            [patch, partial(patch, True)],
            "You have mods saved. Would you like to install them?",
        )
    else:
        patch()
...
def loader_update_required():
    option_list(
        ["Update", "Quit"], [None, quit], "The SpotMod loader is out of date.\n"
    )
...
def uninstall():
    ...
    option_list(
        ["Yes.", "No! Take me back!"],
        [None, main_menu],
        f"{Fore.RED}Are you sure you wish to un-patch Spotify and remove all mods?{Fore.GREEN}",
    )
    utils.clear()
    option_list(
        [
            "Yes, keep them.",
            f"No, delete them. {Fore.RED}(PERMANENT){Fore.GREEN}",
            "Cancel",
        ],
        [
